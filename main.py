...
def index():
    ...
    return str(nowval)+"<br><a href='m' target=_blank>bitcoinfor</a><br><a href='e' target=_blank>bitcoinerror</a>"
...
def m():
    with sqlite3.connect("bitcoinfor.db") as cx1:
        ...
        try:
            ...
            return render_template('m', data=data)
...
def e():
    with sqlite3.connect("bitcoinerror.db") as cx2:
        ...
        try:
            ...
            return render_template('e', newdata=newdata)
        except Exception as e:
            return str(e)

@app.route('/api/q/<int:pk_int>')
def int2pk(pk_int):
    privatekey = int_to_privatekey(pk_int)[1]
    publickey = _get_bitcoin_address(privatekey)[1]
    return jsonify({'pk_int':pk_int, 'prik':privatekey, 'pubk':publickey})

@app.route('/api/d/<int:pk_int>')
def int2del(pk_int):
    with sqlite3.connect("bitcoinerror.db") as cx2:
        cu2 = cx2.cursor()
        try:
            cu2.execute("delete from errortable where now_int="+str(pk_int))
            cx2.commit()
